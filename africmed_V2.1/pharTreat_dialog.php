...
$("#addTreat").click(function(){
	...
	if($("#treatmentType").val()!="Select One"  && !val && $("#drugTreatQuantity").val() != ""){		
		$.post('check.php',{'treatmentPnumber':$('#pharPnumber').val(),'treatmentCategory': $("#treatment option:selected").val(),'treatmentType': $("#treatmentType option:selected").text(),'prescription':$("#treatmentPrescription").val(),'drugQuantity':$("#drugTreatQuantity").val()},function(data) {
...
$("td").delegate('a','click',function(){
	...
	$.post('check.php',{'deleteTreatmentById':id,'pn':$('#pharPnumber').val()},function(data){
		$("#addedTreat").empty().html(data);
		//alert(data);
	});
