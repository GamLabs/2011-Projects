...
require_once 'includes/receptionFunctions.php';
	 ...
	 */
	$aColumns = array( 'date', 'pnumber', 'totalAmount', 'paidAmount','paid_date' );
	...
	$sWhere = " WHERE $nf paidAmount <  totalAmount  ";
	...
	{
		$col = count($aColumns);
		$sWhere = "WHERE $nf paidAmount <  totalAmount and ";
	...
	*/
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			{
				if(!isPrivate($aRow[ $aColumns[$i] ])){
					$status ="Reg: ". getStatusName($aRow[ $aColumns[$i] ]);
				}else{
					$status = "Private";
				}
