...
require_once 'includes/receptionFunctions.php';
	 ...
	 */
	$aColumns = array( 'date','reason','type', 'account_code','amount');
	
	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "date";
	
	/* DB table to use */
	$sTable = "account_transaction";
	...
	if(isset($_GET['month'])){
		$nv = trim($_GET['month']);
		$nf .= "  month(date) = $nv and ";
	}
	
	 $year = $_GET['year'];
	$sWhere = "WHERE $nf  year(date) = $year and account_code ='$code'";
	
	if ( $_GET['sSearch'] != "" )
	{
		$col = count($aColumns);
		$sWhere = "WHERE $nf year(date) = $year and account_code ='$code' and ";
