...
require_once 'includes/receptionFunctions.php';
	 ...
	 */
	$aColumns = array( 'date', 'pnumber', 'type','nominal_code','amount' );
	...
	*/
	...
	if(isset($_GET['sSearch_0'])){
		...
		$nf = "  nominal_code like '%$nv%' and ";
	}
	
	if(isset($_GET['status'])){
		if($_GET['status'] == "bydate"){
			$date = $_GET['date'];
			$sWhere = "WHERE $nf  date = '$date'";
		}elseif ($_GET['status'] == "bymonth"){
			$month = $_GET['month'];
			 $year = $_GET['year'];
			$sWhere = "WHERE $nf month(date) = $month and year(date) = $year";
