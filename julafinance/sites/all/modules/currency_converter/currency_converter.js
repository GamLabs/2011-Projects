$(document).ready(function() {
	...
	$('#currency_converter input').keyup(function() {
		
		var current_value = this.value;
		var current_index = $('#currency_converter input.form-text').index(this);
		var current_price = $('#currency_converter span').eq(current_index).text() ? $('#currency_converter span').eq(current_index).text() : '1';

		jQuery.each($('#currency_converter input.form-text'), function(i, val) {
			if (i != current_index) {
